...
Run (from bibi-media-monitor/):
      python -m pipeline.run            (normal hourly cycle)
...
def run(no_llm=False):
    ...
    for src in sources:
        ...
        log(f"{name}: {total} items, {len(present)} bibi-candidates"
    ...
    for src in sources:
        name = src["name"]
        ps = per_source[name]
        top_items, top_weight, s_num, s_den = 0, 0, 0.0, 0
        ...
        snaps.append({
            ...
            "topic_items": top_items, "topic_weight": top_weight,
            "attention_share": round(top_weight / ps["total_weight"], 5) if ps["total_weight"] else "",
