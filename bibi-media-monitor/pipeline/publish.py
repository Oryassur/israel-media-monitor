"""Build the static JSON files the dashboard reads (docs/bibi/data/)."""
import json
...
def build():
    ...
    hourly = [
        [s["ts"], s["source"],
         ...
         int(s["topic_items"]), int(s["fetch_ok"])]
    ...
    for s in snaps:
        ...
        if s["fetch_ok"] == "1" or s["fetch_ok"] == 1:
            ...
            if s["attention_share"] != "":
                ...
                d["share_n"] += 1
            if s["mean_sentiment"] != "" and s["topic_weight"] not in ("", "0"):
                w = float(s["topic_weight"])
                d["s_num"] += float(s["mean_sentiment"]) * w
                d["s_den"] += w
        d["items"] = max(d["items"], int(s["topic_items"] or 0))
    daily = [
        [date, src,
